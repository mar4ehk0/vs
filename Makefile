...
init: ## Make full application initialization
	...
	docker compose exec api composer install --ansi --prefer-dist
ifeq (,$(wildcard ./app/.env))
	$(shell cp ./src/.env.example ./src/.env)
	docker compose exec api php artisan key:generate
endif
	#@docker compose exec mysql mysql -uroot -p$(DB_PASSWORD) -e "GRANT ALL PRIVILEGES ON *.* TO '$(DB_USERNAME)';" ;
...
shell: ## Start shell into app container
	@(docker compose run --rm api ash) || true
sh: shell

.PHONY: tinker ti
tinker: ## Start shell into app container (press CTRL+D for restart or CTRL+C for exit)
	@(docker compose run --rm api ash -c "while true; do php artisan tinker; echo Reloading; done") || true
ti: tinker
...
clear: ## Make artisan clear (helps in develop env)
	-docker compose run --rm --no-deps api ash -c "\
